# 📸 Snapshot V2.2: The Assimp Wall

> **Status:** Retired. This version led to the creation of the current C++ Native Muscle.

## 🧠 Retrospective Note
This was the "Breaking Point." We had the MToon shader working and the model looked great, but **Assimp** was a black box that kept eating Kisayo's facial expressions. This version proved that to get a "Soul" (blinking/emotions) into the engine, we had to write our own loader logic.

---

# 🧠 Project Snapshot — Greko VRM Engine v2.2
---
> **TL;DR:** VRM facial morphs exist in the GLB, but Assimp deletes them due to zero-weight facial bones.  
> Solution: hybrid loader (Assimp for body, custom loader for face/morphs).


## 🎯 Core Goal

Render and animate **VRoid (VRM 1.0) characters** in **Panda3D** on **Linux** with a **low-end GPU (GT 710)**, using a **custom MToon shader**, while restoring **facial expressions (blendshapes / morph targets)**—specifically blinking—which are currently missing at runtime.
...
* **Assets:** VRM 1.0 → converted to GLB
...
```
engine/
 ├─ base_engine.py        ← Core loop, UI, lighting, entity system
 ├─ entity.py
 └─ camera_controller.py

vrmmanager/
 ├─ vrm_handler.py        ← Character loader/orchestrator (Assimp-based)
 ├─ vrmFixer.py           ← GLB/VRM JSON patcher
 ├─ verifyVRM.py          ← Morph verification utilities
 └─ behaviours/
    └─ blinker.py         ← Intended blink logic (currently blocked)

assets/
 ├─ kisayo.vrm
 ├─ kisayo_fixed.glb      ← Main test asset (patched VRM)
 └─ other VRoid models
...
## 🚨 The Core Problem — *“The Invisible Face”*
...
1. **Morph targets ARE present in the file**
   ...
   * JSON-level inspection of `kisayo_fixed.glb` confirms:
     ...
     ```
     meshes[].primitives[].targets[]  → EXISTS
     ```
...
* Assimp performs a cleanup / pruning pass:
  ...
  * Deletes “unweighted” facial bones
...
* Traversing scene graph:
  ...
  * `GeomNodes` → no SliderTable
  * `Character`, `AnimBundle` → no facial channels
* LoaderOptions tweaks, animation skipping, node preservation

👉 **None stop Assimp from stripping facial morphs.**
...
## 🛣️ Chosen Direction — Hybrid Loader Architecture ✅
...
Assimp deleting bones should *never* kill morphs — but it does — so we bypass it.

---

## 📌 Next Concrete Step (Active Task)
...
**Status:**
🟡 Rendering stable
🔴 Facial animation blocked (by Assimp)
🟢 Solution path identified and underway


* The **model renders correctly** (geometry, materials, lighting all work).
* **Assimp treats zero-weight bones as invalid** and deletes them, taking the morphs with them.
